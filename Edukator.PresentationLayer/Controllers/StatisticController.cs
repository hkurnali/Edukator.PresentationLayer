...
namespace Edukator.PresentationLayer.Controllers
{
    public class StatisticController : Controller
    {
        public IActionResult Index()
        {
            ...
            ViewBag.getMvcCoursePrice=context.Courses.Where(x=>x.Title=="Mvc").Select(y=>y.Price).FirstOrDefault();

            ViewBag.avgReview=context.Courses.Average(x=>x.Review);
            ViewBag.getOver30Price=context.Courses.Where(x=>x.Price>30).Count();
            ViewBag.lastCoursePrice = context.Courses.Where(x => x.CourseID == 17).Select(y => y.Price).FirstOrDefault();
            ViewBag.getTitleByCourseID= context.Courses.Where(x => x.CategoryID == 2).Select(y => y.Title).FirstOrDefault();
            ...
            ViewBag.avpPriceSoftwareCourse=context.Courses.Where(x=>x.CategoryID==(context.Categories.Where(y=>y.CategoryName=="Yazılım").Select(z=>z.CategoryID).FirstOrDefault())).Average(w=>w.Price);
